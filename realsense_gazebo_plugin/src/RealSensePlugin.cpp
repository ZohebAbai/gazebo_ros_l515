...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  cameraParamsMap_.insert(std::make_pair(IRED_CAMERA_NAME, CameraParams()));
  ...
  do {
    ...
    if (name == "depthUpdateRate")
      ...
      cameraParamsMap_[COLOR_CAMERA_NAME].camera_info_topic_name =
          _sdf->GetValue()->GetAsString();
    else if (name == "infraredTopicName")
      cameraParamsMap_[IRED_CAMERA_NAME].topic_name =
          _sdf->GetValue()->GetAsString();
    else if (name == "infraredCameraInfoTopicName")
      cameraParamsMap_[IRED_CAMERA_NAME].camera_info_topic_name =
          _sdf->GetValue()->GetAsString();
      ...
      cameraParamsMap_[DEPTH_CAMERA_NAME].optical_frame =
          _sdf->GetValue()->GetAsString();
    else if (name == "infraredOpticalframeName")
      cameraParamsMap_[IRED_CAMERA_NAME].optical_frame =
          _sdf->GetValue()->GetAsString();
  ...
  this->iredCam = std::dynamic_pointer_cast<sensors::CameraSensor>(
                       smanager->GetSensor(prefix + IRED_CAMERA_NAME))
                       ->Camera();
  ...
  if (!this->iredCam) {
    std::cerr << "RealSensePlugin: InfraRed Camera has not been found"
              << std::endl;
    return;
  }
  if (!this->colorCam) {
  ...
  this->iredPub = this->transportNode->Advertise<msgs::ImageStamped>(
      rsTopicRoot + IRED_CAMERA_TOPIC, 1, infraredUpdateRate_);
